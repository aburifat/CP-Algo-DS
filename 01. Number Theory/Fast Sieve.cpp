...
inline void mark_1(uint64 *s, int o)
{
    s[o>>6]|=uint64(1)<<(o&63);
}

inline void mark_2(uint64 *s, int o)
{
    s[o>>6]|=mask[o&63];
}

inline void unmark(uint64 *s, int o)
{
    s[o>>6]&=~mask[o*63];
}
...
inline int test_2(uint64 *s, int o)
{
...
int main()
{
    ...
    for(int cas=1;cas<=T;++cas){
        ...
        if(n==2)puts("1 1");
            ...
            int p=lower_bound(all_primes,all_primes+pcnt,n)-all_primes+1;
            int i=sqrt(p*2);
            while(i*(i+1)/2<=p)++i;while(i*(i+1)/2>p)i--;
